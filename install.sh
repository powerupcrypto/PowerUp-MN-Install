...
puc Masternode installer arguments:
...
# Set these to change the version of puc to install
TARBALLURL="https://github.com/powerupcrypto/powerup/releases/download/1.0.0/powerup-1.0.0-x86_64-linux-gnu.tar.gz"
TARBALLNAME="powerup-1.0.0-x86_64-linux-gnu.tar.gz"
BOOTSTRAPURL=""
BOOTSTRAPARCHIVE=""
BWKVERSION="1.0.0"
...
echo "
 ...
 | the puc Team CANNOT assist you in repairing  |::
...
rm $TARBALLNAME
mv ./pucd /usr/local/bin
mv ./puc-cli /usr/local/bin
mv ./puc-tx /usr/local/bin
rm -rf $TARBALLNAME

# Create .puc directory
mkdir $USERHOME/.puc
...
# Create puc.conf
touch $USERHOME/.puc/puc.conf
cat > $USERHOME/.puc/puc.conf << EOL
${INSTALLERUSED}
rpcuser=${RPCUSER}
...
cat > /etc/systemd/system/puc.service << EOL
[Unit]
Description=pucd
...
ExecStart=/usr/local/bin/pucd -conf=${USERHOME}/.puc/puc.conf -datadir=${USERHOME}/.puc
ExecStop=/usr/local/bin/puc-cli -conf=${USERHOME}/.puc/puc.conf -datadir=${USERHOME}/.puc stop
Restart=on-abort
[Install]
WantedBy=multi-user.target
EOL
sudo systemctl enable puc.service
sudo systemctl start puc.service
